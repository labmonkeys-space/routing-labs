...
checkError "${?}"
echo -n " - Test connectivity to PC4 (192.168.4.11/24): "
docker exec -it clab-frr01-pc1 ping -W 0.5 -c 1 192.168.4.11 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC5 (192.168.5.11/24): "
docker exec -it clab-frr01-pc1 ping -W 0.5 -c 1 192.168.5.11 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC6 (192.168.6.11/24): "
docker exec -it clab-frr01-pc1 ping -W 0.5 -c 1 192.168.6.11 2>&1 > /dev/null
checkError "${?}"
